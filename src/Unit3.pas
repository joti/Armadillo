...
procedure TForm3.DefaultBtnClick(Sender: TObject);
begin
  fi1:=fi1alap;
  fi2:=fi2alap;
  fin:=finalap;
...
procedure TForm3.Fok1KeyPress(Sender: TObject; var Key: Char);
begin
  if aktiv=69 then
   begin
...
procedure TForm3.VegtelenChkClick(Sender: TObject);
begin
...
procedure TForm3.FokaKeyPress(Sender: TObject; var Key: Char);
begin
  if not (Key in ['0'..'9',',',#8]) then Key:=#0 else DefaultBtn.Enabled:=True;
end;
